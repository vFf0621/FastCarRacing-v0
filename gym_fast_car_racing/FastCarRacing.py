...
class FastCarRacing(gym.Env, EzPickle):
    """
    ## Description
    ...
    A top-down 96x96 grayscale image of the car and race track.
    ...
    def __init__(
        ...
        self.observation_space = spaces.Box(
            low=0, high=255, shape=(STATE_H, STATE_W, 1), dtype=np.uint8
        )
    ...
    def step(self, action: Union[np.ndarray, int]):
        ...
        if action is not None:
            if self.continuous:
                ...
                s, g = action[0], action[1]
                self.car.steer(-s)
                if action[1] >= 0:
                    self.car.gas(g)
                    step_reward = 0.2
                
                elif action[1] <-0.1 and speed >= 70:
                    self.car.brake(0-g)
                    step_reward = 0.05
                else:
                    self.car.gas(0)
